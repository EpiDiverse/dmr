...
process "bedtools_unionbedg" {
    ...
    bedtools unionbedg -filler N/A -i ${group1}*.bed ${group2}*.bed |
    cut -f1,3- | awk '{printf "%s\\t%s", \$1,\$2; for(i=3;i<=NF;i++){printf "\\t%1.2f",\$i}; print null}' |
...
process "metilene" {
    ...
    \$bed 1> ${context}/metilene/\$(basename \$bed .bed)/\$(basename \${bed}) 2> ${context}/metilene/\$(basename \$bed .bed)/\$(basename \$bed .bed).log
